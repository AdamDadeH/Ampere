...
import { computeKNNFromCoords, createDriftState, driftNext, KNNGraph, DriftState } from './navigation'
...
export function RiemannNavigator(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    async function initScene(): Promise<void> {
      ...
      // Load data
      const coordData = await window.api.getTrackFeaturesWithCoords()
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        COLOR_DEFAULT.toArray(colorAttr, i * 3)
      }
      ...
      function animate(): void {
        ...
        // Keyboard fly: translate camera + orbit target together
        if (keysDown.size > 0) {
        ...
        if (state.hoveredIndex >= 0 && state.hoveredIndex !== state.playingIndex) {
          COLOR_DEFAULT.toArray(mesh!.instanceColor!.array as Float32Array, state.hoveredIndex * 3)
        ...
        if (state.playingIndex >= 0 && mesh) {
          const pulse = 1.0 + 0.5 * Math.sin(time * Math.PI)
  ...
  useEffect(() => {
    ...
    if (state.playingIndex >= 0) {
      ...
      dummy.updateMatrix()
      mesh.setMatrixAt(state.playingIndex, dummy.matrix)
      COLOR_DEFAULT.toArray(mesh.instanceColor!.array as Float32Array, state.playingIndex * 3)
    }
  ...
  return (
    <div ref={containerRef} className="flex-1 relative bg-black overflow-hidden">
      ...
      <button
        onClick={toggleDrift}
        className={`absolute top-3 left-3 z-10 px-3 py-1.5 text-xs rounded border transition-colors cursor-pointer ${
          drifting
